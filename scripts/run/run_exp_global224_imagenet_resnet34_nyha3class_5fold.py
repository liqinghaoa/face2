"""One-command entry point for the ResNet34 NYHA three-class experiment."""
...
def load_config(path: Path) -> dict:
    if not path.is_file():
        raise FileNotFoundError(f"ResNet34 config file does not exist: {path}")
...
def validate_resnet34_config(config: dict) -> None:
    if config["experiment"]["name"] != EXPECTED_EXPERIMENT_NAME:
        raise ValueError(
            "Unexpected experiment name in ResNet34 config: "
            f"{config['experiment']['name']!r}"
        )
    if str(config["model"]["backbone"]).lower() != "resnet34":
        raise ValueError(
            f"ResNet34 runner requires backbone=resnet34, got "
            f"{config['model']['backbone']!r}"
        )
    if str(config["model"]["pretrained"]).lower() != "imagenet":
        raise ValueError(
            "ResNet34 runner requires ImageNet pretrained weights, got "
            f"{config['model']['pretrained']!r}"
        )
    if int(config["model"]["num_classes"]) != 3:
        raise ValueError("ResNet34 runner requires num_classes=3")
    if bool(config["model"].get("freeze_backbone", False)):
        raise ValueError("ResNet34 baseline requires freeze_backbone=false")
    ...
    for key, expected in expected_train_values.items():
        ...
        if actual != expected:
            raise ValueError(
                f"ResNet34 controlled baseline requires train.{key}={expected!r}, "
                f"got {actual!r}"
            )
    ...
    for key, expected in expected_augmentation.items():
        ...
        if actual is not expected:
            raise ValueError(
                f"ResNet34 controlled baseline requires augmentation.{key}="
...
def main() -> None:
    ...
    print(f"ResNet34 experiment completed: {experiment_dir}")
