"""Validate and summarize pooled outer-held-out predictions for R3DPR nested-refit experiments."""
...
def make_report(config: dict, fold_metrics: pd.DataFrame, metrics: dict, matrix: np.ndarray) -> str:
    data, train, nested = config["data"], config["train"], config["nested_refit"]
    ...
    return f"""# R3DPR {backbone} Nested-Refit Control vs Patient Binary Experiment
...
- The selection-stage model is discarded. A newly initialized ImageNet-pretrained model trains on all 400 development cases for exactly `selected_epoch` epochs.
- Only the refit model predicts its corresponding outer held-out 100 cases. Outer evaluation data are not used for model selection, early stopping, or epoch selection.
...
def summarize(experiment_dir: Path) -> None:
    ...
    if config.get("nested_refit", {}).get("protocol") != "outer_5fold_inner_holdout_refit":
        raise ValueError("Experiment is not a nested-refit run")
    ...
    for fold in range(n_folds):
        ...
        if not prediction_path.is_file() or not metric_path.is_file():
            raise FileNotFoundError(f"Missing completed nested-refit artifacts for fold {fold}: {fold_dir}")
        predictions = pd.read_csv(prediction_path, dtype={"sample_id": "string", "patient_group_id": "string"})
        if set(predictions.get("protocol", [])) != {"outer_5fold_inner_holdout_refit"}:
            raise ValueError(f"Fold {fold} predictions do not declare the nested-refit protocol")
    ...
    print(f"NESTED_REFIT_OOF_SUMMARY_COMPLETED={experiment_dir}")
