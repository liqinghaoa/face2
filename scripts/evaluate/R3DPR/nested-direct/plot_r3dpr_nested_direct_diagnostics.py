"""Plot internal selection diagnostics for R3DPR nested-direct experiments."""
...
def plot_fold(experiment_dir: Path, output_dir: Path, fold: int, selected_epoch: int, best_auc: float) -> None:
    ...
    fig.suptitle(f"Nested-direct | Outer fold {fold} | selected epoch {selected_epoch} | internal val Macro-AUC {best_auc:.3f}", fontsize=9)
...
def main() -> None:
    ...
    print(f"NESTED_DIRECT_DIAGNOSTIC_DIR={output_dir}")
