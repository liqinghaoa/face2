"""RGB-only S1-2 worker for the project's MediaPipe/BiSeNet environment."""
...
def _device(requested: str) -> Any:
    return face_parser.resolve_device(requested)
...
def main() -> int:
    ...
    checkpoint_value = Path(runtime["parser_checkpoint"])
    checkpoint = checkpoint_value if checkpoint_value.is_absolute() else PROJECT_ROOT / checkpoint_value
    checkpoint = checkpoint.resolve()
    device = _device(str(runtime["parser_device"]))
    ...
    with args.manifest.open("r", encoding="utf-8-sig", newline="") as stream:
        selected = [row for row in csv.DictReader(stream) if row["split"] == args.split]
    for index, row in enumerate(selected, start=1):
        sample_id = row["sample_id"]
        sample_dir = output / args.split / sample_id
        ...
        try:
            ...
            record.update(
                {
                    "status": "PASS",
                    "detection_confidence": metadata["selected_detection_confidence"],
                    "detection_route": detection_route,
            ...
            detail = repr(error)
            record["failure_code"] = detail.split(":", 1)[0].replace("RuntimeError('", "").strip("')")
            record["failure_detail"] = detail
        rows.append(record)
        if index == 1 or index % 20 == 0 or index == len(selected):
            print(f"RGB worker {args.split}: {index}/{len(selected)}", flush=True)
    detector.close()
    mesh.close()
    manifest_path = output / f"rgb_worker_{args.split}.csv"
    ...
    (output / f"rgb_worker_{args.split}_summary.json").write_text(
