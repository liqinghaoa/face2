...
def standardized_root(workspace_root: str | Path | None = None) -> Path:
    ...
    cfg = load_so0_config()
    workspace = Path(workspace_root) if workspace_root is not None else cfg.workspace_root
    return workspace / cfg.model["project"]["standardized_asset_root"]
...
def load_assets(step_nm: int = 5, workspace_root: str | Path | None = None) -> SpectralAssets:
    ...
    cfg = load_so0_config()
    if step_nm not in (cfg.reference_step_nm, cfg.production_step_nm):
